...
public class Ensyu7_19 {
	 ...
	 * 		  int型　　　n    削除する要素の個数
	 * 戻り値:int型配列　a    削除完了の配列
	 * 作成日:2023.04.20
	 */
	static int[] aryrmvN(int[] a, int idx,int n) {
		//値を一つ前の配列にコピーするループ
		for(int indexNumber=idx; (indexNumber+n)<a.length;indexNumber++) {
			//値をコピー
			a[indexNumber] = a[indexNumber+n];
		}
		//削除完了した配列を返す
		return a;
	}
